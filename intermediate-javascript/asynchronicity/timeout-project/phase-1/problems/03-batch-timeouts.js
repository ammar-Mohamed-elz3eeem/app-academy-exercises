...
/**
 * This function accepts an array of callbacks and an array of delays
 * in milliseconds. The function should set a timeout for each
 * callback in the array with its corresponding delay. For example,
 * the callback at index 0 should be set with the delay at index 0,
 * the callback at index 1 should be set with the delay at index 1,
 * and so on. The function should return an array containing the
 * Timeout objects for each timeout that was set. You may assume that
 * both array arguments have the same length.
 *
 * @param {Array<CallableFunction>} callbacks
 *     This is the array of callbacks to be used in setTimeout
 *     function.
 *
 * @param {Array<number>} delays
 *     This is the array of delays in milliseconds.
 *
 * @returns
 *     an array containing the Timeout objects for each timeout that
 *     was set. You may assume that both array arguments have the same
 *     length.
 */
function batchTimeouts(callbacks, delays) {
  // Your code here
  return callbacks.map((callback, index) => {
    return setTimeout(callback, delays[index]);
  })
}
