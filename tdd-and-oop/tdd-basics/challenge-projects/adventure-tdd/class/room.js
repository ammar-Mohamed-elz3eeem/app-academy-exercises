...
class Room {
  ...
  exits;
	...
	getRoomInDirection(direction) {
		const foundRoom = this.exits[direction];
		if (!foundRoom) {
			console.log("No room exist on that direction");
			return;
		}
		return foundRoom;
	}
	...
	getItemByName(name) {
		...
		if (!foundItem) {
			console.log("No item with the specified name exist in the room");
			return;
		}
		return foundItem
	}
	...
	getEnemyByName(name) {
		...
		const foundEnemy = enemiesInRoom.find((enemy) => enemy.name === name);
		if (!foundEnemy) {
			console.log("No enemy with the specified name exist in the room");
			return;
		}
		return foundEnemy;
	}
